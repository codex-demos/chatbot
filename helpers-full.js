...
function setProperty(id, property, value) {
  ...
  if (property === "text") {
    console.warn(
      "setProperty: Use setText() for labels or setValue() for inputs.",
    );
    return;
  }
  ...
  if (property in node.style) {
    ...
    console.warn(
      "setProperty: Property '" + property + "' not recognized on #" + id,
    );
  }
}
...
function getProperty(id, property) {
  ...
  console.warn(
    "getProperty: Property '" + property + "' not recognized on #" + id,
  );
  return "";
}
...
function setValue(id, value) {
  ...
  if (tag === "input" || tag === "textarea" || tag === "select") {
    ...
    console.error(
      "setValue: Element with id='" +
        id +
        "' is not an input, textarea, or select.",
    );
  }
}
...
function image(id, url) {
  ...
  if (node && node.tagName.toLowerCase() !== "img") {
    console.error(
      "image: Element with id='" +
        id +
        "' exists but is not an <img>. Replacing it.",
    );
...
function setScreen(screenId) {
  ...
  if (screens.length === 0) {
    console.warn(
      "setScreen: No elements found with id starting with 'screen'.",
    );
  }
...
function setStyle(id, cssText) {
  ...
  if (typeof cssText !== "string") {
    console.error(
      "setStyle: cssText must be a string like 'color: red; font-size: 20px;'",
    );
    return;
  }
